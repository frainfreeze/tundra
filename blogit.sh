...
POSTS=`pwd`/posts
INDEX=`pwd`/README.md
RES_PATH=`pwd`/res/basic
MD_FLAVOUR="markdown_github"
...
while [ "$1" != "" ]; do
    ...
    case $PARAM in
        ...
        -b | --build)
            ...
            pandoc -f markdown_github $INDEX -o "index.html" --template $RES_PATH/index.Thtml --css $RES_PATH/style.css --self-contained --toc --toc-depth 3
            
            # build blog
            cd posts
            for f in *.md; do 
              pandoc "$f" -s -o "${f%.md}.html" --template $RES_PATH/blog.Thtml --css $RES_PATH/style.css --self-contained --toc --toc-depth 3; 
            done
            cd ..
